...
namespace COMSATS_store.Code
{
    public class Helper
    {
        ...
        {
            string html = @"<div id='alertmsgbox' class='alert alert-success page-alert' style='font-weight: bold; font-size:16px; font-family:Century; color:#00a65a;'>
                           <button id='alertclose' class='close' type='button' onclick='closefunction()'>
                           <span aria-hidden='true'>x</span>
                           <span class='sr-only'>Close</span>
                           </button>
        ...
        public static void SendMail(string email, string pwd)
        {
            ...
            smtp.Credentials = new System.Net.NetworkCredential("ciitpurchase@gmail.com", "Purch1234@");
            smtp.EnableSsl = true;
            smtp.DeliveryMethod = SmtpDeliveryMethod.Network;
            smtp.Send(msg);
        }
        public static bool ChangePassword(string oldpwd, string newpwd, string email)
        {
            string query = "update Employee set pwd='"+newpwd+"'where pwd ='" + oldpwd + "'and email='" + email + "'";
            try
            {
                return DB.insertIntoSQL(query);
            }
            catch
            {
                return false;
